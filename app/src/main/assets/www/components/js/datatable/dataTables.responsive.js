...
Responsive.prototype = {
	...
	_columnsVisiblity: function ( breakpoint )
	{
		...
		if(!columns[0].minWidthDesktop){
			for(var j=0; j< columns.length; j++){
				columns[j].minWidthDesktop = columns[j].minWidth;
		}
		}
		
		if(breakpoint == 'mobile-l' || breakpoint == 'mobile-p' || breakpoint == 'tablet-l' || breakpoint == 'tablet-p' ){
			for(var j=0; j< columns.length; j++){
				if(window.innerWidth < 550){
					columns[j].minWidth = 207;
				}
				
			}
		}else{
			for(var j=0; j< columns.length; j++){
					columns[j].minWidth = columns[j].minWidthDesktop;
			}
		}
